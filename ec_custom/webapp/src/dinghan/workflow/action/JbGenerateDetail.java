...
public class JbGenerateDetail implements Action {
	private Log log = LogFactory.getLog(JbGenerateDetail.class.getName());
	
	@SuppressWarnings("finally")
	@Override
	public String execute(RequestInfo request) {
		String returninfo = FAILURE_AND_CONTINUE;
		ArrayList<JiaBan1> jiabanList = null;
		try {
			...
			jiabanList = JiaBan1.queryList(jb_main.getId());
			
			if (nodeid != 602 && nodeid != 601 && nodeid != 1248) {
				for(JiaBan1 j : jiabanList){
					JiaBan1.update(j.getId(), 0);		// 考勤异常的核定状态为0，此时异常信息待核定
				}
			} else if (nodeid == 602 || nodeid == 601 || nodeid == 1248) {
				
				log.error("流程环节ID :: " + nodeid+ " ---- ");
				for(JiaBan1 j : jiabanList){
					JiaBan1.update(j.getId(), 2);		// 修改考勤异常的核定状态为2，此时异常信息有效
				}
				...
				for (int i = 0; i < jb_onelist.size(); i++) {
					...
					jbtemp.setDataid(String.valueOf(jb_main.getId()));
